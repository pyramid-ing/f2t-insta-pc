...
import { getGlobalSettings, saveGlobalSettings } from '@render/api'
...
const Settings: React.FC = () => {
  ...
  const loadSettings = async () => {
    try {
      ...
      if (res.success && res.data) {
        form.setFieldsValue({
          igId: res.data.loginId || '',
          igPw: res.data.loginPassword || '',
        })
      }
    } catch (error) {
  ...
  const handleSaveLogin = async (values: { igId: string; igPw: string }) => {
    setLoading(true)
    try {
      const res = await saveGlobalSettings({ loginId: values.igId, loginPassword: values.igPw })
      if (res.success) {
